...
router.post("/cash", verifyToken, async (req, res) => {
  const { payment } = req.body; // Nhận thông tin thanh toán từ frontend

  try {
    // Lưu thông tin thanh toán
    const paymentRequest = await Payment.create({
      ...payment,
      paymentMethod: "cash", // Đánh dấu là thanh toán tiền mặt
...
router.get("/all", async (req, res) => {
  try {
    const payments = await Payment.find({}).sort({ updatedAt: -1 }).exec();
